...
用户可以在网页完成：创建训练目标、预览并激活周计划、查看今日训练、确认活动匹配、连续对话复盘、记录跑后反馈、运行跨职责评估、审核计划调整、查看周总结，以及集中审计和撤销 Agent 记忆。
...
storage/ + services/ runtime_observability  持久化 Run/Span、保留策略与只读时间线
...
- 提供按需加载的“记忆档案”，集中展示候选原话摘要、正式偏好、周记忆、生命周期和 Execution / Recovery / Plan 的逐条选中或排除原因；确认、停用和失效继续经过原服务校验。
...
- 打开“记忆档案”，确认或忽略候选、停用长期偏好、标记周记忆失效，并查看三个 Agent 职责实际能读取哪些记忆及原因。
...
当前长期偏好只支持会被周计划真实消费的长跑星期。聊天可以从“以后长跑优先安排在周日”等明确表达中提出待确认候选，但不会自动写入；临时、否定或歧义表达不会生成候选。可用以下命令检查不同职责的正式记忆可见范围：
