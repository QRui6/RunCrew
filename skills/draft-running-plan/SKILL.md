...
description: 根据 RunCrew 中已保存的训练目标、可训练星期、已确认长期偏好、有效周训练记忆、规范化历史活动和恢复评估动作，生成可回放的周训练计划草案或待用户确认的计划调整提案参数。用于用户要求安排下一训练周、根据疲劳或疼痛反馈降低下一课、把下一课改为休息，或检查计划 Agent 是否越权时。不得用于自动批准、直接覆盖激活计划、比赛周处方、伤病诊断或治疗建议。
---
...
- 优先使用目标下最近且仍为 `active` 的 Weekly Training Memory 计算已确认周均时长；至少有 2 个完整周且周均达到 45 分钟时才作为充分基线；
- 周训练记忆不足时，才回退到知识截止日前的规范化活动计算历史周均时长；v1 最多增加 5%；
