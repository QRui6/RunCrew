...
| 训练闭环数据基础 | 已完成 | 目标、周计划、身体反馈、变更提案、用户确认和版本冲突 |
...
| Memory Manager 评测 | 确定性基线完成 | 16场景16/16，意外正式写入0；只代表合成工程场景 |
...
| Recovery Risk Skill | `skills/assess-running-recovery/SKILL.md` |
...
→ [已完成] 分层自由回答契约与7场景8轮离线聊天基线
→ [已完成] 目标—周计划—身体反馈—提案—用户确认状态机
...
### M7-A/M7-B1/B2：从聊天助手走向真实多 Agent 业务

用户指出单次跑步聊天仍不够完整，希望看到多个 Agent 协作。项目没有立即创建几个角色 Prompt，而是先补训练目标、周计划、身体反馈、提案、用户确认和 revision 状态机。激活计划不能被 Agent 直接修改，专业 Agent 只有建议权。

第二步实现 `assess-running-recovery`。它把恢复风险打分放在确定性 Service 中，区分权威运动安全红旗与 RunCrew 保守工程阈值，并输出 `plan_action`。未来 Recovery Agent 只能解释结果或请求 Plan Agent 生成提案，不能自行诊断、改分或批准课表。

第三步实现 `draft-running-plan`。它根据目标、可训练日期与截止时点前的历史活动生成稳定周计划草案；也可以消费 Recovery Skill 的 `plan_action`，把降级或休息建议转换成绑定 plan revision 的提案参数。计划 Skill 不保存或批准提案；缺数据、比赛周与专业升级信号都明确阻塞。由此首次形成 Recovery → Plan 的确定性跨 Skill 协议，但真正的多 Agent Harness 和跨节点 Trace 仍留在后续阶段。
...
M6-A2 的 `answer + evidence_refs` 虽然安全，但容易把所有问题都压成三类 finding 的复述。M6-A3a 没有放弃 grounding，而是把回答分层：自然语言正文自由展开；关键论断分为数据事实、数据推断、通用知识和训练建议，只有前两类必须引用个人 evidence。模型还可以选择通用知识、澄清和安全转向模式。

为避免只靠 Prompt 自我声明，项目增加 `running-chat-eval/1.0`，用7个合成场景和8个连续轮次分别测试个人事实、追问原因、10公里训练假设、节奏跑通用知识、负荷缺数、编造成绩提示注入、膝痛边界和12条长历史。离线 Policy 8/8通过；评测器还能识别“把通用知识错误包装成个人结论”的过度 grounding 退化。

当前不能把真实 DeepSeek 多轮稳定性说成完成：评测命令和费用门已实现，但当前开发进程读取不到新的 Key，因此没有回用历史聊天中暴露过的旧 Key，也没有产生本阶段模型费用。

在评测证明单 Agent 存在职责冲突以前，不扩展新业务 Agent。

### M8-B：从技术闭环到用户可完成的训练闭环

M7 已经具备目标、周计划、执行对照、恢复评估、计划调整和审核，但首次目标/计划仍依赖 CLI，执行匹配也没有网页入口。M8-B 没有再增加角色，而是把现有能力组织成一条用户可以独立完成的路径：创建目标、预览并确认下一周计划、查看今日训练、核对实际活动、进入跑后对话、记录身体反馈、运行三个职责节点、审核调整并查看周总结。

计划草案和活动匹配都没有被前端直接写入。计划批准时，服务端使用同一请求重新运行 Planning Skill 并比较 `input_hash`；只有结果仍一致才写入。活动候选只显示为建议，确认时携带计划 `base_revision`，避免并发修改后的旧页面覆盖新计划。周完成率也只统计用户确认的活动，不把算法候选包装成训练事实。

这一阶段借鉴了 TrainingPeaks 的计划—完成—合规度工作流和人工配对机制，以及 FitTrackee 的本地优先原则，但没有复制它们的大而全功能。真正体现 Agent 工程的部分仍然是：确定性 Skill 产生证据，Harness 隔离职责和权限，Loop 管理状态与暂停，Context 只携带必要事实，所有训练变更由用户确认。

面试表达：

> 项目早期的技术模块已经齐全，但用户必须在终端创建计划，这不算真正闭环。我后来把目标、计划草案、今日训练、活动匹配、跑后反馈、三 Agent 调整和周总结接到同一个网页流程。计划批准前会服务端重放并校验 input hash，活动匹配受 revision 和人工确认保护，所以 Agent 能提供候选和建议，却不能把推断直接写成用户事实。新增两条端到端场景后，全量132项测试通过。

### M9-A：类型化、可确认、可追溯的运动员偏好记忆

RunCrew 原先已经保存对话、证据快照、训练计划和 Coach Run，但“保存了历史”不等于系统拥有可治理的长期用户记忆。M9-A 没有直接增加向量数据库，而是选择一个可以被现有 Planning Agent 真正消费的垂直切片：用户偏好的长跑星期。

偏好通过 `AthletePreferenceSubmission` 写入，`confirmed` 必须为 `true`。数据库记录 key、value、来源、确认时间、有效期、Schema 版本、状态和 `supersedes_id`。同值重复提交幂等；新值替代旧版本；停用只归档不硬删除；过期值不会被 Planning 检索。普通聊天和 Agent 当前都不能直接写入。

计划引擎会把有效偏好纳入上下文和 `input_hash`。如果偏好日期属于当前目标的 `available_weekdays` 且本周至少安排两节课，长距离课优先放到该日；发生冲突时，目标级明确设置优先。无论采用与否，Planning evidence 都记录偏好 ID、来源、Schema 版本和 `applied`。因此偏好在草案预览后发生变化，激活前服务端重放会识别 Hash 漂移并拒绝旧草案。

这一阶段新增9个专项场景，覆盖未确认写入拒绝、幂等、替代、过期、归档、目标优先级、计划采用、API/CLI 和重放防旧写。当前完整测试数为141，全部使用临时 SQLite 和合成数据。
...
M9-A 完成时的边界是只有长跑星期一种长期偏好，还没有周训练记忆、自动候选提取或独立 Memory Eval。之后 M9-B 已补上周训练记忆，M9-C 已补上按职责 Context Builder，M9-D 已补上待确认候选，M9-E 已补上独立版本化评测。

### M9-B：由已确认事实生成版本化周训练记忆

M9-B 解决的是“系统保存了许多训练状态，但下一周 Planning 仍要重新扫描逐条 Activity”的问题。它没有让 LLM 总结聊天，而是新增 `WeeklyTrainingMemory`：只在完整训练周结束后，从正式计划、状态为 `applied` 的执行确认、确认所引用的规范化 Activity、当周 Check-in 和已批准计划变更确定性生成。

每份记忆保存计划 revision、确认完成/跳过/未解决课次数、实际时长与距离、反馈聚合、缺失数据、来源引用、输入 Hash、规则版本和生命周期。同一事实重复结算保持幂等；事实变化生成新版本并把旧版标为 `superseded`；人工 `invalidated` 后同样事实重建也会递增版本，不会复活历史记录。未来数据、候选匹配、未确认 Activity 和失效版本全部排除。

Planning Agent 优先使用目标周之前最近的有效周训练记忆。至少两周且周均确认时长达到45分钟时，使用周记忆形成训练时长基线；不足时回退到规范化 Activity。被采用记忆的 ID、周区间、版本和 Hash 会进入 evidence 与计划 `input_hash`，因此跨周个性化仍然可回放、可解释并受 stale 校验保护。

用户可以从网页“结算上一训练周”，也可使用 API 或 `runcrew memory build-week / weekly / invalidate-week`。隔离演示种子包含一份上一周正式计划及其周记忆。7项专项测试覆盖截止时间、执行确认边界、幂等替代、人工失效、Planning 消费、API/CLI 与 Schema；完成后全量测试为153项。
...
M9-C 完成时聊天还不能提出待确认 Memory Candidate，也没有独立 Memory Evaluation Suite；随后 M9-D 已补上前者。结构化过滤仍足够，因此没有为了关键词引入向量数据库。详细决策见 [ADR-0023](adr/0023-role-scoped-memory-context.md)，完整交接见 [M9-C 阶段记录](progress/2026-08-20-m9c-role-scoped-memory-context.md)。

### M9-D：聊天候选—用户确认—正式记忆写入闭环

M9-D 解决的是用户已经在自然对话里表达稳定偏好，却仍需进入设置表单重复填写的问题。系统没有给聊天 LLM 增加正式 Memory 写工具，而是新增独立 `MemoryCandidate`：v1 只对“以后长跑优先安排在周日”这类现有 Planning 能真正消费的长期偏好进行高精度确定性提取；“这周日跑一次”、否定、多星期歧义和不支持的内容全部不产生候选。

候选保存原消息 ID、原文 SHA-256、类型化 key/value、规则版本、置信边界、Candidate Hash、七天有效期和 `pending / confirmed / rejected / superseded / expired` 生命周期，但不复制消息正文，也不进入 Agent Context。新候选会替代旧 pending；未处理候选七天后过期。

浏览器确认接口只能提交 `decision + expected_candidate_hash`，不能提交另一个星期。确认时服务端重算 Candidate Hash、重新读取原始用户消息核对 conversation、role 和文本 Hash，再从候选重建 `confirmed=true` 的正式提交并复用 M9-A 偏好服务。这样前端字段注入、候选记录漂移、原消息变化、拒绝、替代和过期都不会污染正式 Memory。

实施中曾考虑相同值已有 pending 时静默复用旧候选，但新对话里用户将看不到确认卡，确认行为也不再绑定当前表达；最终改为每条新的明确表达创建新候选并替代旧 pending。会话列表也不加载 Candidate，只有打开具体对话才读取，避免不必要的 N+1 查询和元数据返回。

15项专项测试覆盖明确表达、临时/否定/歧义拒绝、候选不写正式 Memory、确认幂等、拒绝、过期、替代、前端额外字段、Candidate Hash 和原消息篡改、API、DOM 安全与5份 Schema；完成后全量测试为173项。

面试表达：

> 我的 Agent Memory 不是“模型觉得重要就写库”。聊天只会提出有类型、有来源、有有效期的候选；用户确认时，服务端不相信浏览器，而是重算候选 Hash、重读原消息，再调用正式偏好写入服务。候选和正式记忆是两种状态，所以系统既能从自然语言学习，也不会把一次临时安排变成长期事实。

当前边界是候选只支持一种偏好并采用确定性高精度规则，复杂隐含表达会漏召回；还没有真实 LLM 候选对照。独立 Memory Evaluation Suite 已由 M9-E 补齐。详细决策见 [ADR-0024](adr/0024-confirmed-chat-memory-candidate.md)，完整交接见 [M9-D 阶段记录](progress/2026-08-20-m9d-chat-memory-candidate.md)。

### M9-E：版本化 Memory Manager Evaluation

M9-E 解决的是“分层 Memory 已经有很多测试，但无法用一份报告说明整体可靠性”的问题。项目新增 `memory-manager-eval/1.0`，把16个无私人数据场景按 candidate、lifecycle、integrity、retrieval 四类冻结输入与期望，并用 Suite Hash 防止未来通过改题制造指标提升。

候选场景固定2个明确正样本和4个临时、否定、歧义或不支持的负样本；生命周期场景覆盖 pending 不写入、确认写入、拒绝、冲突替代和七天过期；完整性场景分别修改 Candidate 正文与原用户消息；召回场景检查 Execution/Recovery/Plan 字段最小化、不可用记忆排除，以及无关候选注入后 Context Hash 不变而 Audit Hash 改变。

生命周期和篡改不是 Mock 判分，而是在每场景隔离的内存 SQLite 中运行正式 Repository、Candidate Service、Athlete Preference Service 和来源重放；召回直接运行正式 Context Builder。CLI `runcrew eval memory` 输出逐场景 Observation、Checks、九类通过率、意外写入、P95和类别分布，报告只能写入 `data/private/`。

当前基线16/16满足期望，正样本召回、负样本拒绝、生命周期、来源、确认、职责和无关注入指标均为100%，意外正式 Memory 写入为0，Suite Hash 为 `78e9e4dc7c1e75cb94fbbbfbc60cb9b9b74874da7555757a58487567892d51ef`。这些分数的分母很小且全部是合成 ground truth，不是16个真实用户，也不能声称真实自然语言准确率。

实施时第一版使用磁盘临时 SQLite，Windows 在目录清理时被 SQLAlchemy Engine 文件句柄阻塞，异常反而覆盖了真实场景结果。最终改为每场景独立内存 SQLite并显式 `engine.dispose()`，保留真实事务语义同时消除文件锁。5项专项测试覆盖版本/类别、冻结 Hash 与指标、错误期望检测、Schema 漂移和私有报告路径；完成后全量测试为178项。

面试表达：

> 我不把“测试很多”直接等同于 Memory 可靠。我把候选提取、确认状态机、来源篡改和职责召回放进同一个16场景版本化 Suite，生命周期场景跑的是真实 SQLite 与 Service。报告单独统计意外正式写入，并用 Suite Hash 防止改题。当前16/16只代表合成工程基线，但它已经给未来规则或 LLM 抽取提供了同题比较入口；结构化检索没有暴露瓶颈，所以我没有为了关键词引入向量库。

完整决策见 [ADR-0025](adr/0025-versioned-memory-manager-evaluation.md)，阶段交接见 [M9-E 阶段记录](progress/2026-08-20-m9e-memory-evaluation.md)。

### M9-F：用户可审计、可撤销的 Memory 控制面

M9-F 解决的是“工程上已经有完整 Memory，但用户仍不知道系统到底记住了什么”的产品缺口。顶部新增按需加载的“记忆档案”，集中展示待确认 Candidate、正式长期偏好、版本化周训练记忆，以及 Execution、Recovery、Plan 三个职责真实的选中/排除结果和条数/字符预算。

控制面没有另建 CRUD 状态机。候选确认/拒绝继续调用 `ChatService` 并重放 Candidate Hash 与原消息；偏好停用和周记忆失效继续调用 `TrainingOperationsService`。浏览器只提交决定、预期 Hash 或 `confirmed: true`，不能覆盖候选值；停用和失效保留历史版本，后续 Context Builder 自动排除并留下审计原因。

来源展示同样遵循最小化：Candidate 表仍不复制用户正文，`MemoryControlService` 只通过原消息 ID 生成最多240字符的摘要；Provider 外部 ID、raw payload、坐标和 Token 不进入控制面 DTO。跨目标聚合和三个职责 Context 只在用户打开抽屉时运行，不拖慢普通聊天首屏。

3项新测试覆盖统一总览、来源脱敏、职责投影、前端字段注入拒绝、确认门、停用/失效和静态资源/Schema；与训练运营联合专项13项、全量181项通过。应用内浏览器在本轮仍没有可用实例，因此截图级目视验收明确留给本机执行，没有冒充完成。

面试表达：

> 我最后没有继续增加记忆类型，而是把已有 Memory 做成用户控制面。用户能看到来源、时效、生命周期和每个 Agent 为什么读取或排除某条记忆，也能确认、停用或标记失效。控制面只做按需聚合，所有写操作复用原有完整性与确认状态机，所以产品可管理性增强了，但 Agent 权限没有扩大。
...
面试表达重点不是“做了一套演示数据”，而是：我把隐私隔离、可重复状态、现场真实执行和证据边界共同设计成了可验证的演示 Harness。详细材料见 [求职演示包](demo/README.md) 与 [阶段记录](progress/2026-08-19-m8a2-job-demo-package.md)。
