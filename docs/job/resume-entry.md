...
- 围绕“计划—执行—反馈—调整”构建 Execution / Recovery / Plan 三职责 Agent 与 Coach Harness；用版本化 Tool Manifest/Guardrail 约束角色、参数与 Schema，将 Review/Coach 统一映射为可持久化父子 Runtime Run/Span，并从30天事实重算成功率、拒绝率、重试率与P50/P95。
- 计划和记忆确认时由服务端重放并校验 Hash、原始来源与 revision，阻止前端篡改或过期草案覆盖新状态；Runtime 治理合成基线5/5、单 Agent 真实 DeepSeek 同题12/12、多 Agent 确定性 Harness 18/18、Memory Manager 16/16且意外写入0，项目全量202项自动化测试通过。
...
构建带 Tool Manifest/Guardrail、持久化 Run/Span、跨运行指标、候选确认、职责预算、双 Hash 审计和用户可撤销控制面的 Agent Runtime/Memory；Runtime 治理合成基线5/5，单 Agent 真实 DeepSeek 同题12/12，多 Agent 确定性 Harness 18/18，Memory Manager 16/16且意外写入0，全量202项自动化测试通过。
...
- “RAG/向量记忆”：当前长期偏好和周记忆使用类型化 SQLite 与职责 Context Builder，并未引入向量检索。
