...
|---|---|---|---|
| 全量202项自动化测试通过 | `scripts/verify.py`、`tests/` | `.venv\Scripts\python.exe scripts\verify.py` | 202个真实用户场景或模型准确率 |
...
| 四个 Agent 工具由统一 Manifest/Guardrail 治理 | ADR-0027、`tests/test_runtime_governance.py`、M10-A 阶段记录 | 运行 Runtime Governance 与两套 Harness 专项测试 | 生产级安全网关、真实 LLM 攻防或通用工具沙箱结论 |
| Review/Coach 使用统一持久化 Runtime Run/Span | ADR-0028、`tests/test_runtime_observability.py`、M10-B 阶段记录 | 运行 Runtime Observability 专项或只读 Runtime API | 已有分布式 Trace、全路径覆盖、生产告警或观测完整率结论 |
| Runtime 事实可重算跨运行指标，治理合成基线5/5 | ADR-0029、`tests/test_runtime_observability.py`、`tests/test_runtime_evaluation.py`、M10-C 阶段记录 | `runcrew eval runtime-governance` 与 `/engineering` | 生产监控、观测完整率、真实 LLM 攻防或用户效果 |
...
它代表当前仓库全量 pytest 回归，包括领域模型、存储、Provider、Skill、Harness、Runtime Governance/Observability/Metrics、Evaluation、Web/API、Memory 控制面和 Demo Seed。它不是202个评测问题，也不是准确率。
