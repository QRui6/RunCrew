...
|---|---|---|---|
| 全量195项自动化测试通过 | `scripts/verify.py`、`tests/` | `.venv\Scripts\python.exe scripts\verify.py` | 195个真实用户场景或模型准确率 |
...
| Review/Coach 使用统一持久化 Runtime Run/Span | ADR-0028、`tests/test_runtime_observability.py`、M10-B 阶段记录 | 运行 Runtime Observability 专项或只读 Runtime API | 已有分布式 Trace、全路径覆盖、生产告警或跨运行指标结论 |
...
它代表当前仓库全量 pytest 回归，包括领域模型、存储、Provider、Skill、Harness、Runtime Governance/Observability、Evaluation、Web/API、Memory 控制面和 Demo Seed。它不是195个评测问题，也不是准确率。
