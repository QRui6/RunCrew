...
|---|---|---|---|
| 全量146项自动化测试通过 | `scripts/verify.py`、`tests/` | `.venv\Scripts\python.exe scripts\verify.py` | 146个真实用户场景或模型准确率 |
...
它代表当前仓库全量 pytest 回归，包括领域模型、存储、Provider、Skill、Harness、Evaluation、Web/API、Memory 和 Demo Seed。它不是146个评测问题，也不是准确率。
