...
- [x] M9-D：聊天只生成待确认 Memory Candidate，不允许 LLM 直接写入；
...
- [x] M10-C：跨运行指标、版本化治理评测和只读工程观测视图。
