...
**M5 与 M6-A1/A2/A3a 已完成；真实 DeepSeek 聊天同题验收仍待补。M7-A 至 M7-E 已全部完成：聊天产品可运行并审核 Coach 训练闭环，18场景版本化多 Agent 确定性基线为18/18；M8-A1.3 已把两栏产品升级为“运动编辑部 × 数据实验室”式个人跑步办公室，M8-A1.3a/3b 已修复顶部导航和底部输入区的视口裁切，下一步是本机主观视觉复核，通过后进入 M8-A2 求职演示包。**
...
- 桌面根页面已固定为 `68px` 顶栏与剩余工作区两行网格，侧栏和聊天区允许在剩余行内收缩，静态资源版本为 `20260816-6`；
...
**M8-A1.4：本机完成编辑式产品界面的主观视觉验收。**

重启本地服务后复核桌面首屏、产生一轮长回答，并分别打开“查看依据”和“训练运营”抽屉；若信息层级和产品气质通过，则进入 M8-A2，绘制架构图、训练闭环时序图并编写不依赖私人数据的可重复演示脚本。
