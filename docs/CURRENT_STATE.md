...
**M5 与 M6-A1/A2/A3a 已完成；真实 DeepSeek 聊天同题验收仍待新 Key。M7-A 至 M7-E 已全部完成：聊天产品可运行并审核 Coach 训练闭环，18场景版本化多 Agent 确定性基线为18/18；M8-A1.2 已完成两栏首屏、克制型暖灰/炭黑/森林绿视觉与静态资源刷新修复，待重启旧服务并完成本机视觉复核后进入 M8-A2 求职演示包。**
...
- M8-A1.1 已把默认三栏收敛为训练记录＋连续对话两栏；活动快照、智能体协作、用量和安全边界改为按需“运行详情”抽屉；
...
- 新界面保持既有 API、`textContent` DOM 安全边界和响应式布局，JavaScript 语法、专项静态资源测试及 130 项全量测试通过；
- 2026-08-15 应用内浏览器仍无可用实例，因此收敛版视觉点击验收仍需本机人工复核，没有冒充完成截图验收；
...
**M8-A2：基于正式产品界面制作可复现的求职演示包。**

先完成本机视觉复核，再基于已完成实现绘制一张系统架构图和一张训练闭环时序图，编写不依赖私人数据、可在面试现场重复运行的演示脚本。演示必须展示 Activity/Skill/Coach/Handoff/Trace/Evaluation/用户确认与 stale 边界，并明确哪些是确定性基线、哪些真实 LLM 能力尚未验收。M6-A3b 真实 DeepSeek 8轮评测仍待新 Key，不阻塞 M8-A2。
