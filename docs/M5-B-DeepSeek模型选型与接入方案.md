...
> 状态：M5-B1 与 M5-B2 已完成；完整评测的 Hash 与时间预算问题已修复，v1.1 最终同题对照待复跑。
...
最终决策不是把超时排除在 Hash 外，而是将 Suite 升级为 `review-agent-eval/1.1`，让确定性 Policy 和 DeepSeek 都使用相同的15秒总预算。v1.1 确定性基线已经 12/12 通过；正式结论以 DeepSeek 对 v1.1 的同 Hash 复跑为准。被总超时取消的模型请求会记录已发起尝试，但在没有 usage 时不伪造 Token 或费用。
