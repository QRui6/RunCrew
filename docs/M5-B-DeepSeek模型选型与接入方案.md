...
> 状态：M5-B1 至 M5-B3 已完成；v1.1 最终同题对照12/12通过。
...
复核时发现完整命令为了容纳外部延迟，把 v1.0 默认的1秒总超时改成了60秒。这使真实报告的 `suite_hash` 与确定性基线不同。移除改写后的第二次运行虽然 Hash 一致，但9个真实模型场景都在1秒内超时，说明原离线预算本身不适合网络模型。

最终决策不是把超时排除在 Hash 外，而是将 Suite 升级为 `review-agent-eval/1.1`，让确定性 Policy 和 DeepSeek 都使用相同的15秒总预算。最终双方同 Hash 12/12通过。被总超时取消的模型请求会记录已发起尝试，但在没有 usage 时不伪造 Token 或费用。

## 6.3 M5-B3 最终结果

最终 DeepSeek 与确定性基线使用相同 `review-agent-eval/1.1` 和 Suite Hash `2b89473f...`，双方均为12/12满足预期。DeepSeek 的任务、护栏、Schema 和事实一致率均为100%，没有动作解析错误或越权工具执行；共12次 API 请求、13175 Token、估算费用0.00076208美元，P95单场景耗时4862.875ms。

因此当前不升级 Pro：Flash 已满足动作选择质量门槛。也不拆分多 Agent：相同终态与事实结果说明当前单 Agent 的职责、上下文和工具边界没有形成可观察瓶颈。详细比较见 [M5-B3 DeepSeek 最终评测报告](M5-B3-DeepSeek最终评测报告.md)。
