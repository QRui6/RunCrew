...
- 增加 M8-A2 求职演示包：隔离且可重置的合成数据库、系统架构图、训练闭环时序图和五分钟演示脚本；
- 增加 `runcrew demo-seed`、`DemoSeedSummary`、演示输出 Schema、ADR-0021 和5个专项测试；演示种子不读取个人数据库、COROS/FIT 或模型密钥，也不预置 Agent 结论；
- 增加 M8-A3 求职材料包：推荐简历条目、五个核心难点、14个面试追问和量化结论证据映射，明确区分真实 DeepSeek 单 Agent 结果与确定性多 Agent 基线；
- 增加 M9-A 类型化运动员偏好记忆：当前支持经过显式确认的长跑星期偏好，保存来源、确认时间、有效期、Schema 版本和 `supersedes_id`；
- 增加 `athlete_preferences` 本地表、Repository、Memory Service、网页确认/停用入口和 `runcrew memory` CLI；
- Planning Agent 在目标允许的日期内使用长跑日偏好，并把记忆版本、来源和是否采用写入 `input_hash + evidence`；
- 增加两份训练运营输入 Schema、ADR-0020 和9个偏好记忆专项场景；

### Changed

- 周计划激活前重放现在同时检查已确认偏好；偏好改变后，旧草案会被拒绝；
- 静态资源版本升级为 `20260819-1`；

### Verified

- 全量146项测试、Schema 导出、Python 编译和 JavaScript 语法检查通过；没有读取真实活动、调用 COROS 或产生模型费用；

## 2026-08-16

### Added

- 在网页训练闭环中增加训练目标创建、确定性周计划预览与人工激活，不再要求用户先进入终端完成首次配置；
- 增加今日/下一节训练、周执行进度、活动候选确认、标记跳过、解除关联和本周总结；
- 增加计划草案批准前服务端重放与 `input_hash` 校验，并继续用计划 `revision` 保护执行确认；
- 增加六份训练运营 API Schema 和两条端到端产品测试，全量测试增至132项；

### Changed

- 重构 GitHub 首页 README，补充项目问题定义、核心训练闭环图、量化验证结果、技术分层、首次安装步骤和最新网页训练能力；
- “训练运营”升级为“训练闭环”，跑后反馈、Execution / Recovery / Plan 联合评估、计划调整审核和活动对话复盘形成连续用户流程；
- 静态资源版本升级为 `20260816-7`；

### Verified

- JavaScript 语法、目标—计划—周视图和候选—确认—周总结端到端测试及132项全量测试通过；本阶段未调用 COROS、DeepSeek 或其他外部付费服务；
